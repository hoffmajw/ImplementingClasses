...
class Point(object):
    ...
    def move_to(self, x, y):
        self.distance_traveled += math.sqrt((self.x - x) ** 2 + (self.y - y)
                                            **2)
        self.x = x
        self.y = y
        self.num_of_moves += 1
    ...
    def move_by(self, dx, dy):
        self.temp_x = self.x + dx
        self.temp_y = self.y + dy
        self.distance_traveled += math.sqrt((self.x - self.temp_x) ** 2 + (
            self.y - self.temp_y)
                                            ** 2)
        self.x += dx
        self.y = self.y + dy
    ...
    def get_distance_from(self, p):
        ...
        x_square = (self.x - p.x) ** 2
        y_square = (self.y - p.y) ** 2
        return math.sqrt(x_square + y_square)

    def get_distance_from_start(self):
        x_square = (self.x - self.stay_x) ** 2
        y_square = (self.y - self.stay_y) ** 2
        return math.sqrt(x_square + y_square)
    ...
    def closer_to(self, p2, p3):
        dist1 = math.sqrt((self.x - p2.x) ** 2 + (self.y - p2.y) ** 2)
        dist2 = math.sqrt((self.x - p3.x) ** 2 + (self.y - p3.y) ** 2)
    ...
    def halfway_to(self, p1):
        half_x = (self.x + p1.x)/2
        half_y = (self.y + p1.y)/2

        half_point = Point(half_x, half_y)
